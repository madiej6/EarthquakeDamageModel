...
def shakemap_get_bldgs(
    bldg_gdb=constants.BuildingCentroids, eventdir=constants.NapaEventDir
):
    ...
    state_names_list = unique_values(
        table=os.path.join(GDB, "census_county_max_mmi_pga_pgv"), field="STATE_NAME"
    )
    ...
    for state in state_names_list:
        ...
        if arcpy.Exists(fc):
            arcpy.management.MakeFeatureLayer(
                os.path.join(bldg_gdb, state), "{}_lyr".format(state)
            )
    ...
    if len(state_names_list) > 1:
        ...
        arcpy.Merge_management(
            ["{}_lyr".format(x) for x in state_names_list], bldgs_output
        )
    else:
        # copy features
