...
def main(
    ...
    for FIPS in tract_FIPS_list:
        ...
        subset_bldgpcts = bldg_percentages_by_tract_df[
            bldg_percentages_by_tract_df["Tract_str"] == FIPS
        ]
        ...
        for BLDG_TYPE in list_bldgtypes:
            ...
            df_vars = dmgfvarsDF[
                (dmgfvarsDF["BLDG_TYPE"] == BLDG_TYPE)
                & (dmgfvarsDF["BUILDINGCO"] == "HC")
            ]
            seiscode = "HC"
            if len(df_vars) == 0:
                df_vars = dmgfvarsDF[
                    (dmgfvarsDF["BLDG_TYPE"] == BLDG_TYPE)
                    & (dmgfvarsDF["BUILDINGCO"] == "MC")
                ]
                seiscode = "MC"
                if len(df_vars) == 0:
                    df_vars = dmgfvarsDF[
                        (dmgfvarsDF["BLDG_TYPE"] == BLDG_TYPE)
                        & (dmgfvarsDF["BUILDINGCO"] == "LC")
                    ]
                    seiscode = "LC"
                    if len(df_vars) == 0:
                        df_vars = dmgfvarsDF[
                            (dmgfvarsDF["BLDG_TYPE"] == BLDG_TYPE)
                            & (dmgfvarsDF["BUILDINGCO"] == "PC")
                        ]
                        seiscode = "PC"
    ...
    tracts.to_file(
        os.path.join(eventdir, "TractLevel_DamageAssessmentModel_Output.shp")
    )
